...
The first lesson in the "Build a Twitter Clone" series, making the case that a one-sentence app idea is a wish rather than a plan, and that diagrams are the cheapest place to discover a wrong decision. It introduces a three-lens modelling framework — behaviour (flowchart), structure (functional diagram), and interaction (sequence diagram) — each lens derived from the previous, applied to a scoped messaging app called `Bird`.
